...
import type { MessageTarget } from "@openzhiyu/integrations/channels";
import type {
  ExtractedMessageInfo,
  DialogInfo,
} from "@openzhiyu/integrations/channels/sources/types";
import { timeBeforeHours } from "@openzhiyu/shared";
...
export class IMessageAdapter extends MessagePlatformAdapter {
   ...
   */
  private isImageMessage(message: openzhiyuMessage): message is Image {
   ...
   */
  private isFileMessage(message: openzhiyuMessage): message is FileMessage {
    if (typeof message !== "object" || message === null) return false;
    return (
      "id" in message &&
      "name" in message &&
   ...
   * Creates openzhiyu-imessage subdirectory for storing temp files
   */
  private async getTempDir(): Promise<string> {
    const tempDir = join(tmpdir(), "openzhiyu-imessage");
    try {
