...
use openzhiyu_lib::update::{get_platform_download_filename, is_newer_version, parse_semver};
...
mod get_platform_download_filename_tests {
    ...
    fn test_get_platform_download_filename_macos_x64() {
        ...
        assert!(
            name.contains("openzhiyu_1.0.0_macOS_x64.dmg"),
            "got: {}",
            name
        );
    }
    ...
    fn test_get_platform_download_filename_linux() {
        ...
        assert!(result.is_some());
        let name = result.unwrap();
        assert!(name.contains("openzhiyu_1.0.0_linux"), "got: {}", name);
    }
    ...
    fn test_get_platform_download_filename_windows() {
        ...
        let name = result.unwrap();
        assert!(
            name.contains("openzhiyu_1.0.0_windows_x64-setup.exe"),
            "got: {}",
            name
        );
    }
