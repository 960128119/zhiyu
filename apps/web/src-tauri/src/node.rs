...
pub fn find_system_node(home: &str) -> Option<String> {
    ...
    let fixed_paths: Vec<String> = vec![
        format!("{}/.openzhiyu/node/bin/node", env_home),
...
pub fn find_system_node(home: &str) -> Option<String> {
    ...
    // First check fixed paths (including .openzhiyu downloaded node)
    let fixed_paths: Vec<String> = vec![
        format!(r"{}\.openzhiyu\node\node.exe", env_home),
        ...
        .output()
    {
        if output.status.success() {
            ...
            if !path_str.is_empty() && std::path::Path::new(&path_str).exists() {
                if !path_str.contains(".openzhiyu") && is_node_version_valid(&path_str) {
                    println!("✅ Found valid Node.js: {}", path_str);
                    return Some(path_str);
                } else if path_str.contains(".openzhiyu") {
                    println!("⚠️  .openzhiyu Node.js is below v22, will download new version...");
                } else {
        ...
        .output()
    {
        if output.status.success() {
            ...
            for line in path_str.lines().take(5) {
                ...
                if node_path.is_empty() || node_path.contains(".openzhiyu") {
                    continue;
                }
                if std::path::Path::new(node_path).exists() {
...
pub fn download_and_install_node(home: &str) -> Option<String> {
    ...
    let env_home = home.to_string();
    let install_dir = PathBuf::from(&env_home).join(".openzhiyu").join("node");
...
pub fn try_start_nextjs(
    ...
    // If node_cmd contains .openzhiyu, prepend its directory to PATH
    let effective_path = if node_cmd.contains(".openzhiyu") {
    ...
    let cmd = cmd
        ...
        .env("CLOUD_API_URL", "https://app.openzhiyu.ai")
        .env("NEXT_PUBLIC_CLOUD_API_URL", "https://app.openzhiyu.ai")
        .env("TAURI_DB_PATH", db_path)
    ...
    let stderr_path = std::env::temp_dir().join("openzhiyu_node_stderr.log");
...
pub fn start_nextjs_server() {
    ...
    if !server_script.exists() {
        let msg = format!(
            ...
            Please reinstall openzhiyu from openzhiyu.ai",
            server_script
        );
        eprintln!("❌ {}", msg);
        set_startup_error(msg);
    ...
    #[cfg(unix)]
    {
        ...
        let openzhiyu_node_path = format!("{}/.openzhiyu/node/bin", env_home);
    ...
    #[cfg(windows)]
    {
        let openzhiyu_node_dir = format!(r"{}\.openzhiyu\node", env_home);
    ...
    let code_tmpdir = env_home_path.join(".cache").join("openzhiyu-tmp");
