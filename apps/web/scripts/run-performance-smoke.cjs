...
const sessionCookie = process.env.PERF_SMOKE_COOKIE?.trim();
const protectedStatus = sessionCookie ? 200 : [200, 401];

const checks = [
  {
    route: "/knowledge-pipeline?view=sources",
    expectedStatus: 200,
    thresholdMs: pageThresholdMs,
  },
...
async function runPass(label) {
  ...
  for (const check of checks) {
    ...
    console.log(
      `[perf-smoke] ${check.route} -> ${result.status} (${result.elapsedMs}ms, ${result.bytes} bytes)${result.finalUrl !== `${baseUrl}${check.route}` ? ` final=${result.finalUrl}` : ""}`,
    );
  }
  return results;
}
...
async function main() {
  ...
  console.log(
    `[perf-smoke] mode=${sessionCookie ? "authenticated business paths" : "unauthenticated proxy/auth paths"}`,
  );
  console.log(
