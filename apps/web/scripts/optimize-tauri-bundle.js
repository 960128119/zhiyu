...
if (!appBundle) {
  appBundle = path.join(
    webDir,
    "src-tauri/target/release/bundle/macos/openzhiyu.app",
  );
}
...
if (fs.existsSync(infoPlist)) {
  ...
  try {
    ...
    if (currentId !== "com.openzhiyu.app") {
      execSync(
        `/usr/libexec/PlistBuddy -c "Set :CFBundleIdentifier com.openzhiyu.app" "${infoPlist}"`,
        { stdio: "pipe" },
      );
      console.log("  Bundle identifier set to: com.openzhiyu.app");
    }
  } catch {}

  if (!SKIP_SIGNING) {
    const mainBinary = path.join(appBundle, "Contents/MacOS/openzhiyu");
    ...
    if (fs.existsSync(mainBinary) && fs.existsSync(mainEntitlements)) {
      ...
      execSync(
        `codesign --deep --force --sign "${SIGNING_IDENTITY}" --options runtime --timestamp --identifier "com.openzhiyu.app" --entitlements "${mainEntitlements}" "${mainBinary}"`,
        { stdio: "pipe" },
      );
