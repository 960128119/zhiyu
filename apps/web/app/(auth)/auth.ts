...
import { createDevSession, isAuthBypassEnabled } from "@/lib/auth/dev-auth";
...
function createAuthModule(): AuthModuleLike {
  if (isAuthBypassEnabled()) {
    return {
      handlers: {
        GET: async () => Response.json(createDevSession()),
        POST: async () => Response.json(createDevSession()),
      },
      auth: async () => createDevSession(),
      signIn: async () => ({
        ok: true,
        status: 200,
        error: null,
