...
export function useInsightTabs() {
  ...
  const storageKey = useMemo(() => {
    ...
    return `openzhiyu:insight:usertabs:${userId}`;
  ...
  useEffect(() => {
    ...
    window.addEventListener("openzhiyu:tabs-update", loadTabs);

    // Cleanup function
    return () => {
      isMounted = false;
      window.removeEventListener("storage", loadTabs);
      window.removeEventListener("openzhiyu:tabs-update", loadTabs);
    };
  ...
  const saveTabs = useCallback(
    (newTabs: InsightTab[]) => {
      ...
      try {
        ...
        window.dispatchEvent(new CustomEvent("openzhiyu:tabs-update"));
      } catch (error) {
        console.error(
