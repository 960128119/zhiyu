...
function titleAliasesForRequiredField(field: string): string[] {
  const aliases: Record<string, string[]> = {
    accountstate: ["account state", "account status", "账户状态"],
    openingmovereview: ["opening move review", "开盘确认", "开盘走势"],
    tradeplanledger: [
      "trade plan ledger",
      "plan ledger",
      "交易计划",
    ...
    rotationdecision: ["rotation decision", "轮动决策", "调仓决策", "决策"],
    actiontaken: ["action taken", "actions taken", "执行动作", "操作结果", "决策"],
    riskassessment: ["risk assessment", "风险评估", "关键风险", "风险"],
    learningfeedback: ["learning feedback", "学习反馈", "复盘反馈", "决策"],
  };
  const normalized = normalizeFieldName(field);
  return [field, ...(aliases[normalized] ?? [])];
}

function extractMarkdownSections(text: string): Array<{
  title: string;
  body: string;
}> {
...
function extractRequiredFieldFromMarkdown(
  text: string,
  field: string,
): string | undefined {
  const sections = extractMarkdownSections(text);
  const aliases = titleAliasesForRequiredField(field).map((alias) =>
    normalizeFieldName(alias),
  ).filter(Boolean);
  const match = sections.find((section) => {
    const normalizedTitle = normalizeFieldName(section.title);
    return aliases.some(
      (alias) => normalizedTitle === alias || normalizedTitle.includes(alias),
    );
  });
  const value = match?.body || match?.title;
  return value?.trim() || undefined;
}

function repairTrendFollowDecision(text: string) {
  const trendText =
    extractRequiredFieldFromMarkdown(text, "trendStateReview") ||
    extractRequiredFieldFromMarkdown(text, "trendFollowDecision") ||
    firstNonEmptyLine(text) ||
    "The model did not emit a structured trend decision.";
    ...
    candidateDecisions: [
      {
        ...
        blockedReason:
          "Repaired from an unstructured final report; no auditable symbol-level structured decision was emitted.",
        breakWarningHandling:
          "No trade is authorized from repaired prose; keep existing controls and rerun with symbol-level structured trend decisions.",
        nextVerification: "rerun the loop with exact structured-output fields",
        evidence: truncateText(trendText, 400),
      },
    ],
    orders: [],
    blockedReasons: [
      "The final report was markdown/prose instead of a structured trendFollowDecision object.",
    ],
    nextVerification: ["rerun with exact structured-output contract"],
  };
}

function applyRequiredFieldRepairs(input: {
  structured: StructuredExecutionOutput;
  requiredFields: string[];
  text: string;
  warnings: string[];
}): StructuredExecutionOutput {
  if (input.requiredFields.length === 0) return input.structured;

  const repaired: StructuredExecutionOutput = { ...input.structured };
  let repairedAnyField = false;
  let repairedFromFullReport = false;
  const fallbackReport = truncateText(input.text, 1200);

  for (const field of input.requiredFields) {
    const existing = repaired[field];
    if (existing !== undefined && existing !== null && existing !== "") {
      continue;
    }

    if (/^trend(Follow|Trade)Decision$/i.test(field)) {
      repaired[field] = repairTrendFollowDecision(input.text);
      repairedAnyField = true;
      input.warnings.push("trend_follow_decision_repaired_as_blocked");
      continue;
    }

    const extracted = extractRequiredFieldFromMarkdown(input.text, field);
    if (extracted) {
      repaired[field] = extracted;
      repairedAnyField = true;
    } else if (fallbackReport) {
      repaired[field] =
        `Repaired from the full markdown report because no dedicated "${field}" section was emitted.\n\n${fallbackReport}`;
      repairedAnyField = true;
      repairedFromFullReport = true;
    }
  }

  if (repairedAnyField) {
    input.warnings.push("required_fields_repaired_from_markdown");
  }
  if (repairedFromFullReport) {
    input.warnings.push("required_fields_repaired_from_full_report");
  }
...
export function buildStructuredExecutionReport(
  ...
  let structured = input.structuredData ?? {};
  const warnings: string[] = [...(structured.diagnostics?.warnings ?? [])];
  structured = applyRequiredFieldRepairs({
    structured,
    requiredFields: input.requiredFields ?? [],
    text: [input.cleanText, input.rawText].filter(Boolean).join("\n"),
    warnings,
  });
...
export function parseStructuredOutput(rawText: string): ParsedStructuredOutput {
  ...
  if (!tagMatch || tagStart === -1) {
    return parseLastFencedJsonOutput(rawText);
  }
...
function parseLastFencedJsonOutput(rawText: string): ParsedStructuredOutput {
  const fences = [
    ...rawText.matchAll(/```(?:json|JSON)\s*\n([\s\S]*?)```/g),
  ];
  ...
  try {
    ...
    if (result.success) {
      ...
      console.warn(
        "[parseStructuredOutput] Fenced JSON schema validation failed:",
        result.error.issues,
      );
    }
  } catch (e) {
    if (process.env.NODE_ENV === "development") {
      console.warn("[parseStructuredOutput] Fenced JSON parse failed:", e);
    }
    return { data: {}, cleanText: rawText };
  }

  const cleanText = (
    rawText.slice(0, fence.index) +
    rawText.slice(fence.index + fence[0].length)
  ).trim();
  return { data, cleanText };
}
