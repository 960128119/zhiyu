 ...
 * - Unix: ~/.openzhiyu
 * - Windows: %USERPROFILE%\.openzhiyu or %APPDATA%\openzhiyu
 *
 * Note: This function is only available in Node.js environment
 */
export function getAppDataDir(): string {
  ...
  if (process.platform === "win32") {
    ...
    if (userprofile) {
      return join(userprofile, ".openzhiyu");
    }
    return join(process.env.APPDATA || home, "openzhiyu");
  }

  // Unix (Linux/macOS): ~/.openzhiyu
