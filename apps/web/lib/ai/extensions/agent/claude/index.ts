...
function spawnClaudeCodeProcess(options: {
  ...
  const spawnProcess = spawn as unknown as (...args: unknown[]) => any;
  const execProcess = exec as unknown as (...args: unknown[]) => void;
  const os = platform();
  ...
  const registerWindowsTreeKill = (childProcess: { pid?: number }) => {
  ...
  if (isBundledClaude) {
    ...
    const childProcess = spawnProcess(nodeToUse, [cliJsPath, ...options.args], {
      cwd: resolvedCwd,
  ...
  const childProcess = spawnProcess(options.command, options.args, {
    cwd: resolvedCwd,
    env: options.env,
    stdio: ['pipe', 'pipe', 'pipe'],
    signal: options.signal,
...
async function installClaudeCode(): Promise<boolean> {
  ...
  try {
    if (os === 'darwin') {
      ...
      console.error(
        '[Claude] After installation, please restart Zhiyu to continue.',
      );
      return false;
    }
...
function getClaudeCodePath(): string | undefined {
  ...
  if (os === 'win32') {
    ...
    console.warn(
      '[Claude] After installing, restart Zhiyu for the changes to take effect.',
    );
  } else {
    console.warn(
  ...
  if (!path) {
    ...
    if (installed) {
      ...
      if (path) {
        ...
        console.error(
          '[Claude] Please restart Zhiyu after installation completes',
        );
      }
    } else {
      console.error(
...
function expandPath(inputPath: string): string {
  let result = expandHomePath(inputPath);
...
7. **createLoopTask** - Use when user asks for RECURRING, SCHEDULED, BACKGROUND, REMINDER, or MONITOR tasks:
   ...
   **CRITICAL: All new scheduled/background tasks must be native Zhiyu Loops.**
   ...
   - If the task belongs to an existing workshop/agent/Work, pass workshopId or workshopName so the Loop is created inside that workshop. Never create a global Loop when the user names a workshop but the target is unresolved.
...
5. **searchUnifiedMemory** - Use for broad memory recall across raw messages, insights, knowledge, and confirmed Owner Context from external channels:
...
10. **updateLoopTask / setLoopTaskStatus / deleteLoopTask** - Use when managing existing scheduled/background loop tasks:
    - User wants to change a task's time, cadence, name, or objective -> use updateLoopTask.
    - User wants to pause, resume, or archive -> use setLoopTaskStatus.
    - User explicitly wants to permanently delete a non-workshop task -> use deleteLoopTask.
    - Never modify a task by deleting it and creating a new one. Workshop-owned tasks must preserve their workshopId, Skill binding, verifier fields, memory, runs, and audit history.
    - If a workshop task was accidentally deleted, recreate it with createLoopTask and an explicit workshopId/workshopName; otherwise it becomes an orphan global task.
    - Requires loopId; use listLoopTasks first if the user did not provide it.

11. **listWorkshopManifestCapabilities / reviewWorkshopManifestYaml / applyWorkshopManifestYaml** - Use when the user wants to create a new intelligent agent workshop, agent factory, Work, or Workshop from an idea:
   - First use listWorkshopManifestCapabilities to inspect the real available tool and skill names.
   - Guide the user through: controlled object, objective, observation inputs, state/memory, allowed actions, denied actions, approval points, feedback signal, loop cadence, and failure/manual takeover path.
   - Generate a complete \`apiVersion: openzhiyu.ai/v1alpha1\`, \`kind: Workshop\` YAML manifest.
   - Call reviewWorkshopManifestYaml before presenting the manifest as usable. Fix every error and rerun review until ok=true.
   - Default dangerous powers to denied or requiresApproval. Never grant real-money trading, external payment, silent deletion, or external send without an explicit user requirement and a review boundary.
   - Show the reviewed YAML and review summary to the user.
   - Only call applyWorkshopManifestYaml after the user explicitly confirms creation/apply/write-in. Do not create the workshop for exploratory brainstorming or when the user only asks for a YAML draft.

12. **sendReply** - Use when user says:
...
13. **wechatDesktopHealth / wechatDesktopPreviewMessage / wechatDesktopSendMessage** - Use when the user specifically asks to send messages through the local Windows desktop WeChat client:
...
14. **getRawMessages / searchRawMessages** - Use for keyword-based querying of stored message history:
   ...
   - If you use this tool for project/status/risk/owner/next-action questions, also call searchUnifiedMemory to retrieve semantic matches from raw memory, insights, knowledge, and confirmed Owner Context.
...
export class ClaudeAgent extends BaseAgent {
  ...
  private buildEnvConfig(): Record<string, string> {
    ...
    // This is necessary when Zhiyu itself is running inside a Claude Code environment
...
${formattedMessages}${truncationNotice}\n\n---\n## Current Request\n`;
  ...
  async *run(
    ...
    // User's custom API settings from Zhiyu settings page are passed via env config
...
`;
  ...
  private buildWechatDesktopPreviewConfirmation(output: string): string | null {
    try {
      ...
      const recipientName =
        typeof preview?.recipientName === 'string' ? preview.recipientName : '';
      const message =
      ...
      // biome-ignore lint/correctness/noUnreachable: keep the existing fallback text while avoiding unrelated churn.
      return [
        '微信消息已生成预览，还没有发送。',
        '',
        `收件人：${recipientName}`,
