...
export function createUnifiedMemorySearchTool(
  ...
  return tool(
    "searchUnifiedMemory",
    [
      "Primary semantic memory recall tool. Search the owner's context semantically across reviewed Brain memory, raw message fallback, extracted insights, uploaded knowledge, confirmed Owner Context from external channels, and Graph RAG entity-relation context.",
      "",
      ...
      "- The relevant source is unclear or may span memory, insights, knowledge, and Owner Context",
      "",
      "**Search strategy:**",
      "- Start with sources=['memory','insights','knowledge','interactions','graph'] unless the user clearly asks for only one source",
      "- Use sources=['memory'] for reviewed Brain memory first, with raw chat/message history as fallback",
      ...
      "- Use sources=['interactions'] for confirmed facts remembered from external channels such as WeChat",
      "- Use sources=['graph'] for people, projects, commitments, risks, decisions, contradictions, and evidence-linked relationship context",
    ...
    ].join("\n"),
    {
      ...
      sources: z
        ...
        .optional()
        .describe(
          "Sources to search: memory, insights, knowledge, interactions, graph. Defaults to all sources.",
        ),
      limit: z.coerce
        .number()
        .int()
