...
- dialog: (sleep 18 && osascript -e 'display dialog "Notification content" buttons {"OK"} default button 1 with title "Zhiyu Reminder"') &
- notification: (sleep 18 && osascript -e 'display notification "Notification content" with title "Zhiyu Reminder"') &`;
        platformNotificationWorkflow =
          "5. macOS notification - ALWAYS use the delayed notification pattern: (sleep 18 && osascript -e 'display notification ...' with title \"Zhiyu Reminder\") & AND also (sleep 18 && osascript -e 'display dialog ...' with title \"Zhiyu Reminder\") & - BOTH commands must be used with proper background delay execution (the & at the end makes it run in background)";
...
- Use the Bash tool to send system notification: (sleep 18 && notify-send "Zhiyu Reminder" "Notification content") &
- Alternative: (sleep 18 && zenity --info --text="Notification content" --title="Zhiyu Reminder") &`;
        platformNotificationWorkflow =
          '5. Linux notification - ALWAYS use the delayed notification pattern: (sleep 18 && notify-send "Zhiyu Reminder" "Notification content") & - must run in background with & at the end';
        platformNotificationExamples = `- Task: "Remind me to attend meeting at 9 AM every day" (no platform specified)
...
- Use PowerShell: Start-Process powershell -ArgumentList '-Command', 'Start-Sleep -Seconds 18; Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show(\"Notification content\", \"Zhiyu Reminder\")'`;
        platformNotificationWorkflow =
          "5. Windows notification - ALWAYS use the delayed notification pattern: Start-Process powershell -ArgumentList '-Command', 'Start-Sleep -Seconds 18; Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show(\"Notification content\", \"Zhiyu Reminder\")' - must run in background";
        platformNotificationExamples = `- Task: "Remind me to attend meeting at 9 AM every day" (no platform specified)
...
${characterContextSection}`,
    ...
    const artifactReconciliation = reconcileExecutionArtifacts({
      sessionDir,
      sessionsRoot: join(getAppDir(), "sessions"),
