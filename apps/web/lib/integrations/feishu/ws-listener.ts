 ...
 * - User chats with "Feishu app/bot"; openzhiyu listens to messages received by the bot and replies as the bot.
...
interface FeishuConnection {
  ...
  /** Cloud token for bot to call AI in Tauri mode (openzhiyu user's cloud token) */
  authToken?: string;
...
/**
 * Start connections for all Feishu bots under a given openzhiyu user (one bot per account, one connection per bot)
