...
interface AgentChatPanelProps {
  ...
  prefillToken?: string;
  /** A message to send immediately after mount (e.g., from onboarding "Chat with Zhiyu" card), sent only once */
...
export function AgentChatPanel({
  ...
  /** Auto-send initialMessageToSend after mount (e.g., from onboarding "Chat with Zhiyu" click): switches to new chat first, then sends, runs only once; if from URL send param, clears after sending */
  ...
  const { data: votes } = useSWR<Array<any>>(
    chatId && messages.length > 0 ? `/api/vote?chatId=${chatId}` : null,
    voteFetcher,
    {
  ...
  return (
    ...
    >
      <div className={cn("relative flex h-full flex-col bg-card")}>
          ...
          {showScrollButton && (isScrolled || !isNearBottom) && (
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-30">
              <Button
                ...
                className="h-9 w-9 rounded-full border-border/60 bg-background shadow-md transition-[background-color,box-shadow] hover:bg-background hover:shadow-lg"
                aria-label={
                  isNearBottom
