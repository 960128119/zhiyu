...
import { Switch } from "@openzhiyu/ui";
import "../i18n";
...
interface SkillsPanelProps {
  ...
  skills?: Skill[];
  directories?: { agent: string; openzhiyu: string } | null;
  isLoading?: boolean;
  onRefresh?: () => void;
  onOpenLocalFolder?: () => void;
  onCreateSkill?: () => void;
...
export function SkillsPanel({
  ...
  const handleOpenLocalFolder = async () => {
    ...
    if (!internalDirectories?.openzhiyu || !isTauri()) return;
    setInternalOpeningFolder(true);
    try {
      await openPathCustom(internalDirectories.openzhiyu);
      toast({
        type: "success",
        description: t(
  ...
  const addSkillDropdown = (disabled = false) => (
    <AddSkillDropdown
      ...
      disabled={disabled || !directories?.openzhiyu || !isTauri()}
    />
  );
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {t(
                  ...
                  "Add skills to ~/.openzhiyu/skills/",
                )}
              </p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 pt-0 pb-6 px-6">
              {skills.map((skill) => (
                <SkillCard
                  ...
                  skill={skill}
                  canDelete={
                    !!directories && skill.source === directories.openzhiyu
                  }
                  canToggle={
                    !!directories && skill.source === directories.openzhiyu
                  }
...
/**
 * Single skill card: avatar on top, name on next line, description multi-line ellipsis; can enable/disable/delete (openzhiyu skills only)
 */
function SkillCard({
  skill,
  canDelete,
  ...
  const isopenzhiyuSkill = canDelete || canToggle;
