...
import type { Attachment, ChatMessage } from "@openzhiyu/shared";
...
async function importTauriEvent(): Promise<FileDropEventModule | null> {
	return null;
}
					...
					<DropdownMenu>
						...
						>
							<DropdownMenuItem
								onClick={async (e) => {
									e.stopPropagation();
									const { openWorkspaceFileInSystemDefaultApp } = await import(
										"@/lib/files/open-workspace-file-locally"
									);
							...
							<DropdownMenuItem
								onClick={async (e) => {
									e.stopPropagation();
									const { revealWorkspaceFileInParentFolder } = await import(
										"@/lib/files/open-workspace-file-locally"
									);
...
function PureMultimodalInput({
	...
	const { data: skillsData } = useSWR<{
		success: boolean;
		skills: SkillItem[];
	}>(isSlashOpen ? "/api/workspace/skills" : null, fetcher, {
		revalidateOnFocus: false,
	});
