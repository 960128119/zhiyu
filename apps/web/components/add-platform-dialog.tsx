			...
			const pollTimer = setInterval(async () => {
				...
				try {
					...
					if (newAccountIds.length > 0) {
						...
						await mutate("/api/page-state/connectors");
	...
	>(() => {
		...
		return {
			...
			dingtalk: {
				...
				description: t(
					"auth.dingtalkDes",
					"Connect DingTalk via Stream mode using Client ID and Client Secret to chat with openzhiyu.",
				),
				icon: "chat-smile",
			...
			qqbot: {
				...
				description: t(
					"auth.qqbotDes",
					"Connect QQ Open Platform using AppID and AppSecret via WebSocket to chat with openzhiyu.",
				),
				icon: "qq",
			...
			github: {
				...
				description: t(
					"auth.githubDes",
					"Connect GitHub repositories to let openzhiyu learn from your code.",
				),
				icon: "github",
			...
			google_docs: {
				...
				description: t(
					"auth.googleDocsDes",
					"Connect Google Docs to watch document changes and update content directly from openzhiyu.",
				),
				icon: "file_text",
				buttonClass: "bg-[#1A73E8] text-white hover:bg-[#1558b0]",
				iconBackground: "bg-[#1A73E8]/10 text-[#1A73E8]",
			...
			hubspot: {
				...
				description: t(
					"auth.hubspotDes",
					"Connect HubSpot to watch pipeline changes and update deal stages from openzhiyu.",
				),
				icon: "orbit",
			...
			outlook_calendar: {
				...
				description: t(
					...
					"Sync Outlook Calendar to see updates and manage events in openzhiyu.",
				),
				icon: "calendar",
				buttonClass: "bg-[#0F7BFF] text-white hover:bg-[#0c62ca]",
				iconBackground: "bg-[#0F7BFF]/10 text-[#0F7BFF]",
			...
			google_calendar: {
				...
				description: t(
					...
					"Sync your events and get reminders directly in openzhiyu.",
				),
				icon: "calendar",
				buttonClass: "bg-[#1A73E8] text-white hover:bg-[#1558b0]",
				iconBackground: "bg-[#1A73E8]/10 text-[#1A73E8]",
			...
			teams: {
				...
				description: t(
					"auth.teamsDes",
					"Connect Teams chats and channels for openzhiyu insights.",
				),
				icon: "panel_left",
			...
			notion: {
				...
				description: t(
					"auth.notionDes",
					"Save files to Notion and let openzhiyu learn from selected pages.",
				),
				icon: "blocks",
				buttonClass: "bg-black text-white hover:bg-neutral-800",
				iconBackground: "bg-neutral-900/10 text-neutral-900",
				...
				<p>
					...
					{isTauri() ? (
						<>
							<button
								type="button"
								onClick={() => openUrl("https://app.openzhiyu.ai/privacy")}
							...
							<button
								type="button"
								onClick={() => openUrl("https://app.openzhiyu.ai/terms")}
								className="text-blue-500 hover:underline bg-transparent border-none cursor-pointer p-0"
							>
								{t("auth.tos")}
							</button>
			...
			{!onSelectPlatform && (
				<>
					<FeishuAuthForm
						...
						onSuccess={() => mutate("/api/page-state/connectors")}
					/>
					<DingTalkAuthForm
