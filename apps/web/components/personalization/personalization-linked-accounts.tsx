...
import { RssAddControls, RssIntegrations } from "@/components/rss-integrations";
import type {
...
import {
	type RssSubscriptionClient,
	useRssSubscriptions,
} from "@/hooks/use-rss-subscriptions";
...
export function PersonalizationLinkedAccounts({
	...
	initialRssSubscriptions?: RssSubscriptionClient[];
}) {
	const { t, i18n } = useTranslation();
	const router = useRouter();
	const { mutate } = useSWRConfig();
	const { subscriptions: rssSubscriptions } = useRssSubscriptions({
		fallbackSubscriptions: initialRssSubscriptions,
		revalidateOnMount: !initialRssSubscriptions,
	});
				...
				<div className="flex flex-col gap-6 w-full px-6 pt-6 pb-6 space-y-4">
					...
					>
						...
						>
							...
							<AccordionContent className="pt-0 px-0 pb-0">
								<PlatformIntegrations
									...
									initialAccounts={integrationAccounts}
								/>
							</AccordionContent>
						</AccordionItem>
						...
						>
							...
							<AccordionContent className="pt-0 px-0 pb-0">
								<RssIntegrations initialSubscriptions={rssSubscriptions} />
							</AccordionContent>
						</AccordionItem>
					</Accordion>
				</div>
									...
									{inlinePlatformView ? (
										...
										<>
											...
											<TabsContent value="rss" className="mt-0">
												<RssAddControls
													initialSubscriptions={rssSubscriptions}
												/>
											</TabsContent>
										</>
									)}
