---
title: Why We Open-Sourced OpenZhiyu AI
date: 2026-05-28
description: Your AI work partner shouldn't also be your landlord.
image: /img/blogs/openzhiyu-open-source.png
---
...
The day we finished building OpenZhiyu's core architecture, we sat around and asked ourselves one question: should this be closed-source or open-source?
...
OpenZhiyu has been local-first from day one. Your data lives on your machine, encrypted, never leaving your device. Nothing goes to our servers. That's not a feature — it's our stance.
...
![Memory Layers Architecture](/img/openzhiyu/memory-layers.png)

### N platforms, one inbox

We've connected Telegram, WhatsApp, WeChat, Slack, Discord, QQ, DingTalk, Feishu, iMessage — all major messaging apps covered, and you can use your phone to chat with OpenZhiyu and remotely trigger anything you want it to do.
...
All of this — messages, emails, calendar events, document updates — flows continuously into OpenZhiyu. Not just push notifications, but AI-understood, organized context. A built-in background Agent runs every 30 minutes like a diligent warehouse keeper, sorting new information, extracting value, so that by the time you talk to the AI, it's already done its homework.
...
Every OpenZhiyu capability — memory system, platform connectors, automation engine, document processing — is packaged as a standalone Skill interface. You can integrate these skills into any Agent you prefer: Claude Code, Codex, OpenClaw, Hermes, and more — as long as it supports skills or a similar extension protocol, it can call everything OpenZhiyu has.

This isn't a lock-in strategy. It's a choice: **The base layer is public infrastructure. The Agent on top is your tool.** Which Agent you use is your call. OpenZhiyu handles memory and tools in the background.
...
But we asked ourselves: **is OpenZhiyu solving a big enough problem?**
...
Building OpenZhiyu internally, our biggest pain wasn't missing features — it was realizing you can only imagine so many use cases when you're building something yourself.
...
**We believe open-source makes us better.** Not because we get free labor from the community — but because the community will turn OpenZhiyu into something we could never have imagined ourselves.
...
**OpenZhiyu is open-source. The installer is free. The AI is yours.**
...
**[→ View OpenZhiyu on GitHub](https://github.com/melandlabs/openzhiyu)**
...
| Platform            | Download                                                                                                   |
| ------------------- | ---------------------------------------------------------------------------------------------------------- |
| macOS Apple Silicon | [.dmg](https://github.com/melandlabs/openzhiyu/releases/download/v0.5.0/openzhiyu_0.5.0_macOS_aarch64.dmg) |
| macOS Intel         | [.dmg](https://github.com/melandlabs/openzhiyu/releases/download/v0.5.0/openzhiyu_0.5.0_macOS_amd64.dmg)   |
| Linux AMD64         | [.deb](https://github.com/melandlabs/openzhiyu/releases/download/v0.5.0/openzhiyu_0.5.0_linux_amd64.deb)   |
| Linux ARM64         | [.deb](https://github.com/melandlabs/openzhiyu/releases/download/v0.5.0/openzhiyu_0.5.0_linux_aarch64.deb) |
| Windows             | [.exe](https://github.com/melandlabs/openzhiyu/releases/download/v0.5.0/openzhiyu_0.5.0_windows_amd64.exe) |

Or clone and run it yourself:

```bash
git clone https://github.com/melandlabs/openzhiyu.git
cd openzhiyu && pnpm install && pnpm tauri:dev
```

For issues or questions, hit [Documents](https://openzhiyu.ai/docs), [GitHub Issues](https://github.com/melandlabs/openzhiyu/issues) or [Discord](https://discord.com/invite/xkJaJyWcsv) — we're around.
