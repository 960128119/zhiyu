...
Paste the automation prompt into OpenZhiyu. It defines the nightly workflow: commit fetching, report compilation, email delivery, and Linear sync.
...
OpenZhiyu executes the full pipeline — fetching commits, compiling the report, and syncing issues — then generates structured artifacts for each output.
...
OpenZhiyu's tracking panel shows exactly how many times each task has run, against which repositories:
