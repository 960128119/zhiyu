---
title: "Memory"
description: "OpenZhiyu memory system — local-first tiered storage with hybrid vector search and automatic lifecycle management"
icon: "FaBrain"
---

# Memory

OpenZhiyu's memory system is a local-first, tiered knowledge base built from messages across connected platforms. Memory grows on its own over time — completely visible and auditable. Unlike other AIs with a "forgetting engine," OpenZhiyu remembers everything that matters, with full transparency into what it knows about you.
...
At its core, OpenZhiyu's memory system builds a **Holistic Context Graph** — a comprehensive representation of your work that captures:
...
Unlike traditional memory systems that merely store and retrieve messages, the Holistic Context Graph builds structured knowledge from raw data through a self-evolving closed loop:

```
Diverse context sources (email, IM, screen, audio)
    ↓
...
OpenZhiyu transforms this pattern for personal AI agents, extracting structured insights from diverse context sources (email, IM, screen, audio) and building the Holistic Context Graph through progressive abstraction.
...
When you ask OpenZhiyu about your memory, the query goes through several layers:
...
The **time dimension** of the Holistic Context Graph solves one of the hardest challenges in agent memory: handling **time-sensitive queries** like "What did I work on last week?" or "Give me a summary of decisions made in January."

Standard retrieval fails because it returns current state without temporal context. OpenZhiyu implements **Temporal Validity** — the ability to see what was relevant at a specific point in time:

```bash
# Get insights valid at a specific point in time
node $SKILL_DIR/scripts/openzhiyu-memory.cjs get-insights-as-of 2024-01-01
...
node $SKILL_DIR/scripts/openzhiyu-memory.cjs get-insights-in-interval 2024-01-01 2026-06-01
```

When querying with a historical timestamp, OpenZhiyu:
...
The **spatial dimension** of the Holistic Context Graph implements **associative memory connections** inspired by the Hebbian learning principle ("neurons that fire together wire together"):
...
The Holistic Context Graph architecture was inspired by closed-loop patterns from Coding Agent workflows. The complete flow:
...
This creates a complete **Holistic Context Graph**: global/local preferences + structured code + GitHub Issue/Kanban + Git History + Coding Agent patterns + immutable raw context + Insights + Timeline + global relationship graph + self-evolving memory.

---

## Memory Evaluation Framework

OpenZhiyu's memory system is evaluated across four key dimensions, ensuring comprehensive assessment of accuracy, recall, temporal reasoning, and knowledge update capabilities:
...
OpenZhiyu achieves **96%+ accuracy** on LoCoMo and LongMemEval-S500 benchmarks, on par with SOTA. See [Benchmarks](/docs/benchmark) for detailed performance data.

---

## Memory as a Skill

Memory is also available as a standalone **Skill** for integration with other Agent systems. This allows any AI agent to connect to OpenZhiyu's memory capabilities and leverage the same tiered storage, vector search, and knowledge base features.
...
| Feature                   | Description                                                                                                               |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| **Memory Files Search**   | Case-insensitive full-text search across local memory files (`~/.openzhiyu/data/memory/`)                                 |
| **Knowledge Base Search** | Semantic document search using RAG/embeddings on the OpenZhiyu server                                                     |
...
- **Memory Files**: Personal markdown/JSON files stored locally at `~/.openzhiyu/data/memory/` with subdirectories for chats, channels, people, projects, notes, and strategy
- **Knowledge Base**: Uploaded documents searchable via RAG/embeddings on the OpenZhiyu server
...
The CLI automatically reads authentication tokens from `~/.openzhiyu/token` (base64 encoded JWT).

For full integration details, visit the [openzhiyu-memory Skill](https://github.com/melandlabs/openzhiyu/blob/main/skills/openzhiyu-memory/SKILL.md).
