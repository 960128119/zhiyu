...
async function waitForDocker(timeoutMs = 120000) {
  ...
  throw new Error("Docker Desktop started, but Docker Engine was not ready in time.");
}
...
async function ensureDockerReady() {
  ...
  log(`Docker Engine is not ready. Starting Docker Desktop from ${dockerDesktopPath}`);
...
async function ensurePostgres() {
  ...
  const exists = tryRun(dockerCommand, ["inspect", "-f", "{{.Name}}", pgContainer]);
  if (exists) {
...
async function ensureWechatService() {
  ...
  if (await checkWechatHealth()) {
    log(`WeChat desktop service already healthy at ${wechatHost}:${wechatPort}`);
    return;
  }
