...
Zhiyu can generate embeddings locally with Transformers.js and store
vectors in either SQLite through `sqlite-vec` or a ChromaDB server.

## Bailian cloud embeddings

For the cloud path, Zhiyu can call Alibaba Cloud Model Studio / DashScope
through its OpenAI-compatible embeddings API. This is the recommended hosted
configuration for Chinese personal-memory and knowledge-base workloads:
...
`DASHSCOPE_API_KEY`, `BAILIAN_API_KEY`, and `ALIBABA_CLOUD_API_KEY` are treated
as equivalent key names. The runtime also accepts environment references such
as `DASHSCOPE_API_KEY=$ANTHROPIC_API_KEY`, so local development can reuse the
same Bailian key without duplicating the secret.

When a Bailian key is present and no embedding model is explicitly configured,
the default cloud embedding model becomes `text-embedding-v4`. The provider
normalizes `https://dashscope.aliyuncs.com` and
`https://dashscope.aliyuncs.com/compatible-mode` to the required
...
The retained SQLite compatibility runtime can use `sqlite-vec`:
...
The native `sqlite-vec` extension is loaded by the Node.js SQLite adapter. It
does not require a separate vector database service. The current Web runtime
uses PostgreSQL as its source of truth and only enables this backend when the
SQLite compatibility mode is active.
...
Zhiyu always supplies embeddings to Chroma explicitly. Chroma's default
