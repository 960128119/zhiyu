...
const TOKEN_PATH = path.join(os.homedir(), '.openzhiyu', 'token');
...
function apiRequest(endpoint, method = 'GET', body = null, portIndex = 0) {
  return new Promise((resolve, reject) => {
    if (portIndex >= PORTS.length) {
      reject(new Error('openzhiyu server not running. Tried ports: ' + PORTS.join(', ')));
      return;
    }
...
async function getOAuthUrl(platform) {
  ...
  const token = getAuthToken();
  if (!token) {
    throw new Error('Not authenticated. Please log in to openzhiyu first.');
  }
...
Examples:
  node openzhiyu-connectors.cjs list-platforms
  node openzhiyu-connectors.cjs list-accounts
  node openzhiyu-connectors.cjs status telegram
  node openzhiyu-connectors.cjs connect gmail --email=my@gmail.com --password=abcdefghijklnop
  node openzhiyu-connectors.cjs disconnect int_xxx
  node openzhiyu-connectors.cjs query-contacts --name=John --page=1 --pageSize=10
  node openzhiyu-connectors.cjs send-reply --botId=bot_xxx --recipients=John --message="Hello!"
          `.trim()
        }, null, 2));
    }
  } catch (error) {
