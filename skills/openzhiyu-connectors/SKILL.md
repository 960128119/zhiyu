---
name: openzhiyu-connectors
description: "openzhiyu Connectors tools - manage platform integrations (OAuth connections, list accounts, check status). Triggers: connect platform, integration status, list accounts, disconnect"
metadata:
  version: 0.5.1
allowed-tools: Bash(node $SKILL_DIR/scripts/openzhiyu-connectors.cjs *)
---
...
# OpenZhiyu Connectors Skill

OpenZhiyu Connectors provides access to 26 messaging and productivity platform integrations. It allows AI agents to manage OAuth connections, list connected accounts, check connection status, and disconnect platforms on behalf of the user.
...
**openzhiyu takes a fundamentally different approach: it operates as a proactive digital partner** that watches, learns, remembers, and acts on your behalf. The difference is architectural.

### How It Works

When users connect messaging platforms and integrations to openzhiyu, they sync with permission:
...
This aggregated data becomes "the single source of truth for openzhiyu's brain."

### The Continuous Sync Loop

openzhiyu runs a background agent on a continuous sync loop, actively gathering information from all connected sources. An agent without this loop can only respond based on stale context. With it, every conversation—and every moment—makes openzhiyu smarter and more aligned with you.
...
Users can also authorize accounts directly through the openzhiyu desktop application without using CLI commands.

### Adding Account Authorization via Desktop UI

1. **Open openzhiyu desktop app** on your computer
...
node $SKILL_DIR/scripts/openzhiyu-connectors.cjs list-platforms

# List all connected accounts (includes botId for send-reply)
node $SKILL_DIR/scripts/openzhiyu-connectors.cjs list-accounts

# Check connection status for a platform
node $SKILL_DIR/scripts/openzhiyu-connectors.cjs status telegram

# Connect a platform (opens browser for OAuth)
node $SKILL_DIR/scripts/openzhiyu-connectors.cjs connect slack

# Disconnect an account by ID
node $SKILL_DIR/scripts/openzhiyu-connectors.cjs disconnect int_xxx

# Query contacts
node $SKILL_DIR/scripts/openzhiyu-connectors.cjs query-contacts --name=John --page=1 --pageSize=10

# Send a message (requires botId from list-accounts)
node $SKILL_DIR/scripts/openzhiyu-connectors.cjs send-reply --botId=bot_xxx --recipients=John --message="Hello!"
```
