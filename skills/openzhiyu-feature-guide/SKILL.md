...
description: "Use this when users ask about openzhiyu features, capabilities, or how to use it. Examples: 'openzhiyu 怎么用', '你能做什么', 'What can you do?', 'How does openzhiyu work?', 'Tell me about openzhiyu features', 'What platforms does openzhiyu support?', 'How do I use scheduled tasks?', 'What is Insights system?', 'How do I connect Telegram?', 'How to create automation?', '什么是 openzhiyu 事件?'
metadata:
  version: 0.5.1
---

> **Note:** If you haven't downloaded or installed openzhiyu yet, please refer to [Getting Started](https://openzhiyu.ai/docs/getting-started) for installation instructions.

# OpenZhiyu Product Features

Use this skill when users ask about openzhiyu features, usage, or capabilities. Provide accurate and easy-to-understand feature introductions and operation guides.

---

## What is OpenZhiyu

openzhiyu is a **Proactive AI Workspace** that understands your intent, orchestrates execution, and gets things done. It's not just another AI assistant—it's an innovative AI product that **senses business signals**, **orchestrates tasks autonomously**, and **tracks and validates results end-to-end**.

### Core Value Proposition

openzhiyu transforms how individuals and SMB teams work by:
...
### 🧠 Long-Term Memory
Clear recollection, never forgotten. openzhiyu builds persistent knowledge graphs that remember all important people, events, decisions, and context across sessions and time. Six months later, it still knows your commitments and preferences.

### 🎯 Noise Filtering
Tells you what you should act on. With hundreds of daily messages, openzhiyu replaces "information overload" with "priority signals." Filters 95% of noise, focusing your attention on the 5% that truly matters.

### ⚡ Powerful Engine
Intent understanding, automatic orchestration. When you say "Help me prepare an investor pitch," openzhiyu automatically understands intent, breaks it into multiple sub-tasks, invokes appropriate Skills, and chains execution.
...
### 🌍 Global Managers
Never miss a critical signal across time zones. Business runs 24/7 globally. openzhiyu filters time zone and language noise, capturing high-value opportunities while you sleep—wake up to a refined action list.
...
### 🚀 Founders & Sales
One person does the work of many, at scale. openzhiyu learns your communication style, automatically maintains hundreds of client relationships, follows up on leads, generates personalized proposals—never burns out.
...
2. Tell openzhiyu what you'd like it to help with
...
3. Once authorized, openzhiyu will automatically read your WhatsApp messages to generate long-term events. You can send messages in **"Starred Messages"** and AI will:

- Read and understand your message content
- Generate smart insights in openzhiyu
- You can converse with AI about these messages in openzhiyu
...
3. Once authorized, openzhiyu will automatically read your Telegram messages to generate long-term events on the Today page. You can send messages in **"Saved Messages"** and AI will:
...
2. Click **[Install openzhiyu]** on the authorization page to add to your workspace
...
3. Grant openzhiyu bot message permissions
...
**Connect in openzhiyu:**

1. Click **[Connect DingTalk]**
2. Enter your **Client ID (AppKey)** and **Client Secret (AppSecret)**
3. Click **Connect**

> 💡 Stream mode means openzhiyu connects directly via WebSocket — no server or domain setup needed.
...
- Add the running app (Terminal, Node, or openzhiyu)
...
openzhiyu also offers a **desktop app** (macOS, Linux, and Windows) that provides a native local experience.

### Download & Install

**Windows:**

1. Download the latest `.exe` installer from [GitHub Releases](https://github.com/melandlabs/openzhiyu/releases/latest)
2. Run the installer — if Windows SmartScreen shows a warning, click **"More info"** then **"Run anyway"**. This is normal for new applications without a long code-signing reputation; openzhiyu is open-source and the code is publicly verifiable
3. After the first run, SmartScreen typically bypasses automatically on subsequent updates

**Winget (Coming Soon):**

```
winget install openzhiyu.openzhiyu
```
...
Download the latest `.dmg` from [GitHub Releases](https://github.com/melandlabs/openzhiyu/releases/latest) and drag openzhiyu to your Applications folder.

**Linux:**

Download the latest `.AppImage` or `.deb` from [GitHub Releases](https://github.com/melandlabs/openzhiyu/releases/latest).
...
When you first launch openzhiyu, the system may ask for a few permissions. Each one has a specific purpose — and **you can decline any of them**. openzhiyu will continue to work; you'll just lose the feature that requires that permission.

> **Privacy note:** openzhiyu processes data locally on your machine. Message content is never sent to third-party servers without your consent. [See our Privacy Policy →](https://app.openzhiyu.ai/privacy)
...
No special permissions are required on Linux. openzhiyu uses the standard desktop notification system (libnotify) to send alerts — if you have granted notification permissions to other apps, openzhiyu will use them automatically.

---

## Conversation Features

Just type your questions or requests in the chat box, and openzhiyu will help you find answers.

### Example Questions

```
• "What is openzhiyu"
• "How to use openzhiyu"
...
- **Web browsing** - Have openzhiyu browse for latest product info
- **New conversation creation** - Start new conversations around specific topics
- **History** - View conversation history
- **Source References** - See exactly which messages/conversations openzhiyu's answers come from, who was involved, and when they occurred
- **Artifacts** - openzhiyu can generate visual artifacts: mind maps, flowcharts, charts, roadmaps, surveys, and documents. Preview and interact with them directly in chat
- **File Analysis** - Upload files (PDF, images, etc.) in chat and ask openzhiyu to analyze, summarize, or extract information
...
You can also interact with openzhiyu directly through your connected messaging apps. Once connected, openzhiyu becomes your AI assistant within those platforms.
...
After connecting (e.g., Telegram), just send a message to **Saved Messages** and openzhiyu will respond naturally.

---

## Action Features

openzhiyu can generate action items for various scenarios.
...
- **Quick action suggestions** - openzhiyu suggests clickable quick actions
...
- **Message replies** - Click reply button in understanding detail view, openzhiyu will generate a reply

---

## Smart Insights

openzhiyu automatically analyzes your conversations to extract valuable information.
...
If you no longer want openzhiyu to access a connected platform, you can disconnect it at any time:
...
Once disconnected:
- openzhiyu will immediately stop reading new messages from that platform
...
Change the language used in openzhiyu.
...
Your data, your sovereignty. openzhiyu puts privacy and control first—you never need to trade data sovereignty for intelligence.

### 🔐 Our Privacy Principles

#### Local-First Architecture
Your original messages and files **stay on your device**. openzhiyu only accesses the minimum data needed to complete tasks—no unnecessary uploads.
- Raw data never leaves your local environment
- Only processed results are transmitted when needed
- Complete control over what data openzhiyu can access
...
openzhiyu gives you complete control over your data:
- **Access Controls**: Choose exactly which platforms and data openzhiyu can access
- **Data Retention**: Configure how long openzhiyu retains processed data
- **Delete Anytime**: Delete all your data at any time with one click
- **Export**: Export all your data in standard formats

We believe **privacy and intelligence aren't mutually exclusive**. You should never have to choose between powerful AI capabilities and data control. openzhiyu delivers both.

---

## FAQ

### Q: How do I use openzhiyu?

A: It's simple!
1. Start a conversation in openzhiyu
...
## Showcase: openzhiyu in Action

Real-world examples showing how openzhiyu works across different use cases.
...
openzhiyu Welcome Bot:
👋 Welcome @newuser!

I'm openzhiyu, your community assistant:
...
- openzhiyu website: https://openzhiyu.ai
- openzhiyu documents: https://openzhiyu.ai/docs
- Terms of Service: https://app.openzhiyu.ai/terms
- Privacy Policy: https://app.openzhiyu.ai/privacy
