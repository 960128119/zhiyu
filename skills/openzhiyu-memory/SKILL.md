...
description: "openzhiyu Memory tools - search memory files, knowledge base, and chat insights. Triggers: memory search, knowledge base, documents, insights"
metadata:
  version: 0.5.1
allowed-tools: Bash(node $SKILL_DIR/scripts/openzhiyu-memory.cjs *)
---
...
# OpenZhiyu Memory Skill

OpenZhiyu Memory is a personal knowledge management tool that searches and manages three types of information:

| Type | Description | Data Location |
|------|-------------|--------------|
| **Memory Files** | Personal memory files (markdown/json) | `~/.openzhiyu/data/memory/` |
| **Knowledge Base** | Uploaded documents via RAG/embeddings | openzhiyu server |
| **Insights** | Structured info extracted from chat history, with usage tracking and automatic maintenance | openzhiyu server |

---

## Overview

openzhiyu Memory is built on a unique architectural principle: instead of treating memory as an afterthought, it's the foundation.

**How it works with Connectors:** Before memory can search your data, you need to connect your platforms using the `openzhiyu-connectors` skill. Connectors handles OAuth authentication and integration setup for 26 platforms (Telegram, WhatsApp, Slack, Discord, Gmail, Outlook, Twitter/X, WeChat, and more). Once connected, openzhiyu continuously syncs everything with your permission—raw messages, meetings, emails, tweets, calendar events, voice calls, and any notes or ideas you've captured. This aggregated data becomes the single source of truth that powers openzhiyu's brain.
...
Inside openzhiyu, memory is layered into multiple levels:
...
The CLI auto-reads your token from `~/.openzhiyu/token` (base64 encoded JWT).
...
Memory files are stored locally at `~/.openzhiyu/data/memory/` and searched via direct filesystem access. This is a **read-only** operation that performs case-insensitive text search across `.md` and `.json` files.

### Directory Structure

```
~/.openzhiyu/data/memory/
...
```bash
node $SKILL_DIR/scripts/openzhiyu-memory.cjs add-memory "Content to remember" --file=filename.md --directory=notes
```
- `--file` (optional): Filename. If not provided, auto-generated from first line of content.
- `--directory` (optional): Subdirectory under `~/.openzhiyu/data/memory/`. Created if doesn't exist.

**Deleting a memory file:**
```bash
node $SKILL_DIR/scripts/openzhiyu-memory.cjs delete-memory filename.md --directory=notes
```

### How search-memory Works

1. **Path**: `~/.openzhiyu/data/memory/` (or subdirectory if specified)
...
openzhiyu tracks insight usage and performs periodic maintenance to preserve retrieval quality, avoiding context decay.
...
node $SKILL_DIR/scripts/openzhiyu-memory.cjs search-all "query"

# Search local memory files (full-text, case-insensitive)
node $SKILL_DIR/scripts/openzhiyu-memory.cjs search-memory "boss"

# Search local memory files in specific subdirectory
node $SKILL_DIR/scripts/openzhiyu-memory.cjs search-memory "project" --directory=projects

# Search knowledge base (RAG, semantic search)
node $SKILL_DIR/scripts/openzhiyu-memory.cjs search-knowledge "project plan"

# List knowledge base documents
node $SKILL_DIR/scripts/openzhiyu-memory.cjs list-documents

# Get document content
node $SKILL_DIR/scripts/openzhiyu-memory.cjs get-document doc_xxx

# List recent insights (last 7 days)
node $SKILL_DIR/scripts/openzhiyu-memory.cjs list-insights --days=7

# List insights from a specific channel (e.g., Gmail, Telegram, WhatsApp)
node $SKILL_DIR/scripts/openzhiyu-memory.cjs list-insights --channel=gmail --days=7
node $SKILL_DIR/scripts/openzhiyu-memory.cjs list-insights --channel=telegram --days=30
node $SKILL_DIR/scripts/openzhiyu-memory.cjs list-insights --channel=whatsapp

# Filter insights by keyword (supports multiple keywords - OR logic)
node $SKILL_DIR/scripts/openzhiyu-memory.cjs list-insights --keyword=screen --keyword=linkedin --days=30

# Get single insight
node $SKILL_DIR/scripts/openzhiyu-memory.cjs get-insight insight_xxx

# Create a new insight
node $SKILL_DIR/scripts/openzhiyu-memory.cjs add-insight --title="Coffee preference" --description="I prefer Americano coffee" --importance=General

# Update an insight (partial update with array append)
node $SKILL_DIR/scripts/openzhiyu-memory.cjs update-insight insight_xxx --description="Updated description" --detail="User mentioned new preference"

# Delete insight
node $SKILL_DIR/scripts/openzhiyu-memory.cjs delete-insight insight_xxx

# Add a memory file
node $SKILL_DIR/scripts/openzhiyu-memory.cjs add-memory "My boss John likes Monday project discussions" --file=people/boss.md

# Delete a memory file
node $SKILL_DIR/scripts/openzhiyu-memory.cjs delete-memory people/boss.md
```
...
| `search-memory` | Full-text search in local `.md`/`.json` files | `~/.openzhiyu/data/memory/` |
| `search-knowledge` | Semantic search via embeddings | openzhiyu server (RAG) |
...
node $SKILL_DIR/scripts/openzhiyu-memory.cjs search-all "person/project/topic"

# Then optionally get details from specific sources
node $SKILL_DIR/scripts/openzhiyu-memory.cjs search-memory "person" --directory=people
node $SKILL_DIR/scripts/openzhiyu-memory.cjs list-insights --days=30 --keyword=<keyword>
```
...
node $SKILL_DIR/scripts/openzhiyu-memory.cjs get-related-insights <insightId>

# Get related insights with filters
node $SKILL_DIR/scripts/openzhiyu-memory.cjs get-related-insights insight_xxx --limit=10 --minStrength=0.3

# Get connection statistics
node $SKILL_DIR/scripts/openzhiyu-memory.cjs get-connection-stats <insightId>
```
...
node $SKILL_DIR/scripts/openzhiyu-memory.cjs get-insights-as-of 2026-01-01

# Get currently valid insights (no expiration or future expiration)
node $SKILL_DIR/scripts/openzhiyu-memory.cjs get-current-insights

# Get insights overlapping a time interval
node $SKILL_DIR/scripts/openzhiyu-memory.cjs get-insights-in-interval 2026-01-01 2026-06-01
```
...
node $SKILL_DIR/scripts/openzhiyu-memory.cjs list-entities --type=person

# Search entities by name
node $SKILL_DIR/scripts/openzhiyu-memory.cjs list-entities --search=John

# Get entity details with linked insights
node $SKILL_DIR/scripts/openzhiyu-memory.cjs get-entity <entityId> --insights
```
...
node $SKILL_DIR/scripts/openzhiyu-memory.cjs search-with-connections "project deadline"

# With custom limit
node $SKILL_DIR/scripts/openzhiyu-memory.cjs search-with-connections "project deadline" --limit=5
```
