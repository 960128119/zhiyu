---
name: openzhiyu-api
description: "openzhiyu API documentation and reference. Use when working with openzhiyu backend APIs, AI, authentication, characters, messages, files, integrations, billing, or any server-side functionality. Triggers: API endpoints, backend routes, authentication, cloud API, integrations"
metadata:
  version: 0.5.1
---
...
```bash
curl -X POST https://app.openzhiyu.ai/api/remote-auth/login \
...
```bash
curl -X POST https://app.openzhiyu.ai/api/ai/chat \
...
When running openzhiyu desktop app, the local API server runs on port **3414** (fallback: **3515**):
...
The auth token is stored at `~/.openzhiyu/token` (base64 encoded JWT). You **must decode it** before use:

```bash
# Decode base64 to get JWT token
TOKEN=$(cat ~/.openzhiyu/token | base64 -d)
...
curl https://app.openzhiyu.ai/api/remote-auth/user \
  -H "Authorization: Bearer $TOKEN"
```
